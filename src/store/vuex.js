    ...
    const store = new Vuex.Store({
        ...
        mutations: {
            ...
            getData(state) {
                ...
                // }
                // if (state.role === 3) {
                    url = `/api/v1/send/my/?status=0&&just_me=false`;
                // }
                let states = state;
            ...
            changePageData(state, currentPage) {
                ...
                // }
                // if (state.role === 3) {
                    url = `/api/v1/send/my/?status=0&&just_me=false&&page=${currentPage}`;
                // }
                let states = state;
                axios.get(url).then(function (e) {
                    states.dispatchData = e.data;
            ...
            getReData(state) {
                ...
                // }
                // if (state.role === 3) {
                    url = `/api/v1/receive/my/?status=0&&just_me=false`;
                // }
                let states = state;
                axios.get(url).then(function (e) {
                    // console.log(e.data)
            ...
            changeRePageData(state, currentPage) {
                ...
                // }
                // if (state.role === 3) {
                    url = `/api/v1/receive/my/?status=0&&just_me=false&&page=${currentPage}`;
                // }
                let states = state;
                axios.get(url).then(function (e) {
                    states.receiveData = e.data;
            ...
            changePageDataUn(state, currentPage) {
                let url = `/api/v1/send/?status=1&&page=${currentPage}`;
                let states = state;
                axios.get(url).then(function (e) {
                    states.dispatchDataUn = e.data;
            ...
            //获取初始收文页码(提交未审批)
            getReDataUn(state) {
                let url = '/api/v1/receive/?status=1';
                let states = state;
                axios.get(url).then(function (e) {
                    // console.log(e.data)
            ...
            changeRePageDataUn(state, currentPage) {
                let url = `/api/v1/receive/?status=1&&page=${currentPage}`;
                let states = state;
                axios.get(url).then(function (e) {
                    states.receiveDataUn = e.data;
            ...
            getNumbers(state) {
                let url = `/api/v1/ref_num/`
                axios.get(url).then(function (e) {
                    state.numbers = e.data;
                    // console.log(e.data)
                })
            },
            //获取初始发文页码(审批完待签发)
            ...
            changePageDataRead(state, currentPage) {
                let url;
                // if (state.role === 2) {
                url = `/api/v1/send/?status=3&&page=${currentPage}`;
                // }
                // if (state.role === 3) {
                //     url = `/api/v1/send/?page=${currentPage}`;
                // }
                let states = state;
                axios.get(url).then(function (e) {
                    states.dispatchDataRead = e.data;
            ...
            changeRePageDataRead(state, currentPage) {
                let url;
                // if (state.role === 2) {
                url = `/api/v1/receive/?status=3&&page=${currentPage}`;
                // }
                // if (state.role === 3) {
                //     url = `/api/v1/receive/?page=${currentPage}`;
                // }
                let states = state;
                axios.get(url).then(function (e) {
                    states.receiveDataRead = e.data;
            ...
            getDataDone(state) {
                ...
                // console.log(state.pageDataDone)
            },
            //点击后获取更新每页信息(已办结）
            ...
            getFormData(state, obj) {
                ...
                state.formData = {
                    ...
                    first_check_leader: obj.first_check_leader,
                    // red: obj.red,
