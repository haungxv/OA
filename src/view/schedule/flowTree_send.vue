    ...
    export default {
        ...
        data() {
            return {
                ...
                control: {
                    ...
                    lineColor: ['gray', 'gray', 'gray', 'gray', 'gray', 'gray', 'gray'],
                    tooltip: [
                        '未进行',
                        '未进行',
                        '未进行',
        ...
        methods: {
            judgeControl() {
                ...
                //发稿人和发稿时间
                this.control.tooltip[0] = `发稿人：${this.data[0].detail.user}<br/>时间：${this.data[0].detail.time.replace('T', ' ').substring(0, 19)}`;
                for (let i = 1; i < 8; i++) {
                    //处理一审和上级审批两个状态
                    if (i === 1) {
