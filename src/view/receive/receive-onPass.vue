<template>
    <div class="whole" style="width: 90%;margin: 0 auto">
        <div class="receive-propose-onPass" v-show=!this.$store.state.ReadReceiveMark>
            <el-table
                    :data="pageDataDone"
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button @click="onRead(scope.row)" type="text"
                                   class="receive-propose-onPass-button"
                                   v-if=isIn
                        >查看
                        </el-button>
                        ...
                        <el-button @click="openPass(scope.row)" type="text"
                                   v-if=isShow
                                   class="receive-propose-onPass-button">查看传阅
                        </el-button>
    ...
    export default {
        ...
        methods: {
            handleCurrentChange(e) {
                this.page = e;
                this.$store.commit('changeRePageDataRead', e);
            },
            onRead(event) {
                this.$store.commit('changeReadReceiveMark');
            ...
            onSubmit() {
                ...
                this.$post(url, formData).then(function (e) {
                    ...
                    self.$store.commit('changeReadReceiveMark')
                }).catch((e) => {
                    self.$message({
                        message: '提交失败',
                        type: 'error',
            ...
            openPass(e) {
                let self = this;
                let url = `/api/v1/receive_sign/${e.id}/`
            ...
            secondSend() {
                ...
                let url = `/api/v1/receive_sign/${this.S_id}/end/`;
                let self = this;
                this.$put(url).then(function () {
                    let urls = `/api/v1/receive_sign/${self.S_id}/second_check/`;
            ...
            endSend() {
                ...
                let self = this;
                this.$put(url).then(function () {
                    let urls = `/api/v1/receive/${self.S_id}/end/`;
                    self.$put(urls).then(function () {
                        self.handleCurrentChange(self.page)
                        self.$message({
                            message: '已办结',
        ...
        computed: {
            //当前页面显示的文单信息
            pageDataDone() {
                let now = this.$store.state.receivePageDataRead;
                if (now) {
                    now.map(function (a) {
                        a.re_term = a.term;
            ...
            //获取总共的发文单长度
            tableDataLengthDone() {
                return this.$store.state.receiveDataRead.count;
            }
        },
        mounted() {
            //获取初始文单信息，展示初始页面
            this.$store.commit('getReDataRead');
