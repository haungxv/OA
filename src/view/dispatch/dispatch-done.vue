<template>
    <div class="dispatch-propose-done" id="Print">
        <el-table
                :data="pageDataDone"
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <el-button @click="onPrint(scope.row)" type="text"
                               class="dispatch-propose-done-button">打印
                    </el-button>
                    <el-button @click="onLoad(scope.row)" type="text"
                    class="dispatch-propose-done-button">下载附件
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            onPrint(event) {
                ...
                //重新写入文档流-->
                // console.log(event.desc_content)
                myWindow.document.write('<html>');
                myWindow.document.write(headHtml);
                // myWindow.document.write(`<head><style media="print">@page { size: auto;margin: 5mm;   }</style></head>`);
        ...
        computed: {
            //当前页面显示的文单信息
            pageDataDone() {
                let now = this.$store.state.pageDataDone;
                if (now) {
                    now.map(function (a) {
            ...
            //获取总共的发文单长度
            tableDataLengthDone() {
                return this.$store.state.dispatchDataDone.count;
            }
        },
        mounted() {
            //获取初始文单信息，展示初始页面
            this.$store.commit('getDataDone');
        }
    }
</script>

<style lang="less" TYPE="text/less">
    .dispatch-propose-done {
        width: 90%;
        margin: 0 auto;
